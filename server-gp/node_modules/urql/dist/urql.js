...
function useSource(input, transform) {
  ...
  var ref = react.useMemo((function() {
    var subject = wonka.makeSubject(), source = wonka.concat([ wonka.fromValue(input), subject.source ]), updateInput = function(nextInput) {
      if (nextInput !== input) {
    ...
    currentInit = !0;
    var state;
    wonka.subscribe((function(value) {
      state = value;
    }))(transform(wonka.fromValue(input))).unsubscribe();
    currentInit = !1;
    return state;
  })), state = ref$1[0], setState = ref$1[1];
  react.useEffect((function() {
    return wonka.subscribe(_ref2)(transform(input$, state)).unsubscribe;
  }), [ input$ ]);
...
function _ref4(query$) {
  ...
  return wonka.concat([ wonka.fromValue({
    fetching: !0,
    stale: !1
  }), wonka.map(_ref3)(query$), wonka.fromValue({
    fetching: !1,
    stale: !1
  }) ]);
}
...
function _ref5(query$$, prevState) {
  return wonka.scan(_ref2, prevState || initialState)(wonka.switchMap(_ref4)(query$$));
}
...
function useQuery(args) {
  var client = useClient(), request = useRequest(args.query, args.variables), makeQuery$ = react.useCallback((function(opts) {
    ...
    if (!source) {
      ...
      if (suspense) {
        sources.set(request.key, source = function toSuspenseSource(source) {
          function _ref(_resolve) {
            resolve = _resolve;
          }
          var cache, resolve, shared = wonka.share(source);
          return function(sink) {
            var signal, hasSuspended = !1;
            wonka.takeWhile((function(result) {
              if (void 0 === cache) {
  ...
  }), [ args.pause, makeQuery$ ]), ref = useSource(query$, react.useCallback(_ref5, [])), state = ref[0], update = ref[1], executeQuery = react.useCallback((function(opts) {
...
function useSubscription(args, handler) {
  ...
  state = (ref = useSource(subscription$ = react.useMemo((function() {
    ...
    return wonka.scan(_ref, prevState || initialState)(wonka.switchMap(_ref3$1)(subscription$$));
  }), [])))[0];
